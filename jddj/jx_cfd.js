...
async function takeGetRequest(type) {
    ...
    switch (type) {
        // 获取用户信息
        case 'QueryUserInfo':
            url = `https://m.jingxi.com/jxcfd/user/QueryUserInfo?strZone=jxcfd&bizCode=jxcfd&source=jxcfd&dwEnv=7&_cfd_t=${Date.now() + 2}&ptag=&ddwTaskId=&strShareId=&_stk=_cfd_t%2CbizCode%2CddwTaskId%2CdwEnv%2Cptag%2Csource%2CstrShareId%2CstrZone&_ste=1`
            url += `&h5st=${decrypt(Date.now(), '', '', url)}&_cfd_t==${Date.now() + 2}&sceneval=2&g_login_type=1&callback=jsonpCBK${String.fromCharCode(Math.floor(Math.random() * 26) + "A".charCodeAt(0))}&g_ty=ls`;
            myRequest = getGetRequest(`QueryUserInfo`, url);
            break;
        // 获取签到列表
        case 'QuerySignListV2':
            url = `https://m.jingxi.com/jxcfd/task/QuerySignListV2?strZone=jxcfd&bizCode=jxcfd&source=jxcfd&dwEnv=7&_cfd_t=${Date.now() + 2}&ptag=7155.9.470.18.28&_stk=_cfd_t%2CbizCode%2CdwEnv%2Cptag%2Csource%2CstrZone&_ste=1`;
            url += `&h5st=${decrypt(Date.now(), '', '', url)}&_cfd_t==${Date.now() + 2}&sceneval=2&g_login_type=1&callback=jsonpCBK${String.fromCharCode(Math.floor(Math.random() * 26) + "A".charCodeAt(0))}&g_ty=ls`;
            myRequest = getGetRequest(`QuerySignListV2`, url);
            break;
        case 'mowing': //割草
...
function dealReturn(type, data) {
    switch (type) {
        case 'QueryUserInfo':
            ...
            if (data.SceneList) {
                $.info.SceneList = data.SceneList;
            } else {
                console.log(`获取用户信息异常\n`);
            }
            break;
        case 'QuerySignListV2':
            let data = JSON.parse(data(data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1]));
            $.info.dwUserFlag = data.sData.dwUserFlag;
            ...
            if (dwStatus === 0) {
                takeGetRequest()
                userSignReward(dwUserFlag, ddwMoney);
            } else {
                $.log(`\n📌签到：你今日已签到过啦，请明天再来`);
            }
            break;
        case 'jump':
            ...
            if (data.ret === 0) {
                ...
                console.log(`横扫鸡腿获得金币：${$.mowingInfo.addcoins} \n`)
            }
            break;
        case 'cow':
            data = JSON.parse(data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1]);
        ...
        case 'feed':
            ...
            if (data.ret === 0) {
                ...
                $.pause = true;
            }else if (data.ret === 2005 && data.data.maintaskId === 'pause') {
                console.log(`投喂失败,${data.data.message}`);
                $.runFeed = false;
            } else {
