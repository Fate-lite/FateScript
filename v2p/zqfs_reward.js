...
需要捉签到和奖励body，分别存在zqkdFastSignBody和zqkdMyRewardBody
...
let rewardBody = ($.isNode() ? process.env.zqkdMyRewardBody : $.getdata('zqkdMyRewardBody')) || '';
let rewardBodyArr = []
...
async function checkEnv() {
    ...
    if(rewardBody) {
        ...
        console.log('未找到zqkdMyRewardBody')
    }
        ...
        if(rewardBody) {
            ...
            for(let i=0; i<rewardBodyArr.length; i++) {
                ...
                if((bodys.indexOf(uidStr) > -1) && (bodys.indexOf(action) > -1)) {
                    console.log(`找到用户[${uid}]重复的任务【${task}】zqkdMyRewardBody: ${body}`)
                    return;
                }
            }
            //没有return，即新body
            rewardBody = rewardBody + '@' + body
            $.setdata(rewardBody, 'zqkdMyRewardBody');
            $.msg(jsname+` 获取到用户[${uid}]任务【${task}】的zqkdMyRewardBody: ${body}`)
        } else {
            $.setdata(body, 'zqkdMyRewardBody');
            $.msg(jsname+` 获取到用户[${uid}]任务【${task}】的zqkdMyRewardBody: ${body}`)
        }
    }
