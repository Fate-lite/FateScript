...
scripts_base_url_1=https://ghproxy.com/https://raw.githubusercontent.com/Fate-lite/FateScript/main/fate/
...
##############################作者脚本名称（必填）##############################
# 将相应作者的脚本填写到以下变量中
my_scripts_list_1="jd_beauty.js
jd_cash.js
jd_factory.js
jd_Fruit.js
jd_jdzz.js
  ...
  do
    ...
    if [ $? -eq 0 ]; then
	  ...
	  if [ -z "${script_date}" ];then
	    cron_min=$(rand 1 59)
	    cron_hour=$(rand 7 9)
	    [ $(grep -c "$croname" /jd/config/crontab.list) -eq 0 ] && sed -i "/hangup/a${cron_min} ${cron_hour} * * * jtask $croname"  /jd/config/crontab.list
	  else
	    [ $(grep -c "$croname" /jd/config/crontab.list) -eq 0 ] && sed -i "/hangup/a${script_date} jtask $croname"  /jd/config/crontab.list
	  fi
    else
