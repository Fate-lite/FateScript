...
[task_local]
#618限时盲盒
30 7,19 1-18 6 * https://raw.githubusercontent.com/Fate-lite/FateScript/main/fate/jd_618Limitbox.js, tag=618限时盲盒, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

================Loon==============
[Script]
cron "30 7,19 1-18 6 *" script-path=https://raw.githubusercontent.com/Fate-lite/FateScript/main/fate/jd_618Limitbox.js tag=618限时盲盒

===============Surge=================
618限时盲盒 = type=cron,cronexp="30 7,19 1-18 6 *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/Fate-lite/FateScript/main/fate/jd_618Limitbox.js

============小火箭=========
618限时盲盒 = type=cron,script-path=https://raw.githubusercontent.com/Fate-lite/FateScript/main/fate/jd_618Limitbox.js, cronexpr="30 7,19 1-18 6 *", timeout=3600, enable=true
...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        if (cookie) {
            ...
            await getcode()
            for (let k = 0; k < $.availableOpenBoxNum; k++) {
                await limitBoxDraw()
            }
            await $.wait(1000);
        }
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        if (cookie) {
            $.index = i + 1;
            console.log(`\n******开始【京东账号${$.index}】\n`);
            for (let l = 0; l < codeList.length; l++) {
...
function taskUrl(functionId, body) {
    return {
        url: `${JD_API_HOST}client.action?functionId=${functionId}&body=${encodeURIComponent(body)}&appid=ld&client=m&clientVersion=10.0.1&networkType=wifi&osVersion=11&uuid=2393039353533623-7383235613364343&openudid=2393039353533623-7383235613364343&eu=2393039353533623&fv=7383235613364343&jsonp=`,
        headers: {
