 ...
 */
const $ = new Env('极速版翻翻乐');
//Node.js用户请在jdCookie.js处填写京东ck;
const jdCookieNode = $.isNode() ? require('./jdCookie.js') : '';
const openum = process.env.JSOPENNUM; // 翻牌次数 可以自己改
    ...
    for (let i = 0; i < $.cookieArr.length; i++) {
        ...
        if (cookie) {
            ...
            if (leftTime != 0) {
                ...
                for (let k = 0; k < openum && $.canDraw; k++) {
...
function check() {
    return new Promise(async (resolve) => {
        ...
        let options = taskUrl("gambleHomePage", body)
...
function totalPrize() {
    return new Promise(async (resolve) => {
        ...
        let options = taskUrl("gamblePrizeList", body)
        $.get(options, async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`);
...
function open(functionid, type) {
    return new Promise(async (resolve) => {
        ...
        if (type) {
            ...
            options = taskPostUrl(functionid, body)
        }
...
function Draw(id, poolBaseId, prizeGroupId, prizeBaseId, prizeType) {
    return new Promise(async (resolve) => {
        ...
        let options = taskPostUrl("apCashWithDraw", body)
        $.post(options, async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`);
...
function taskUrl(function_id, body) {
    return {
        url: `${JD_API_HOST}/?functionId=${function_id}&body=${escape(JSON.stringify(body))}&t=${Date.now()}&appid=activities_platform&clientVersion=3.5.0`,
        headers: {
            "Accept": "*/*",
            ...
            "Host": "api.m.jd.com",
            "Referer": "https://618redpacket.jd.com/?activityId=DA4SkG7NXupA9sksI00L0g&channel=wjicon&sid=f1a44f55dff1f24587447f5cc02f2f5w&un_area=22_1930_49322_49424",
            "Cookie": cookie,
...
function taskPostUrl(functionid, body) {
    return {
        url: JD_API_HOST,
        body: `functionId=${functionid}&body=${escape(JSON.stringify(body))}&t=${Date.now()}&appid=activities_platform&clientVersion=3.5.0`,
        headers: {
            "Accept": "*/*",
            "Accept-Encoding": "gzip, deflate, br",
            "Accept-Language": "zh-cn",
