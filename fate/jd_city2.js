...
[task_local]
#城城领现金
0 0-23/1 * * * https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_city2.js, tag=城城领现金, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

=================================Loon===================================
[Script]
cron "0 0-23/1 * * *" script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_city2.js,tag=城城领现金

===================================Surge================================
城城领现金 = type=cron,cronexp="0 0-23/1 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_city2.js

====================================小火箭=============================
城城领现金 = type=cron,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_city2.js, cronexpr="0 0-23/1 * * *", timeout=3600, enable=true
 */
const $ = new Env('城城领现金内部互助版本');
...
let inviteCodes = [
    'HoHsz-qlQg2-fYOcHtQ2mhEeiKtHzKxG3udpt_fDa0a15S_B',
    'W9GzlJTRIk_cAtbSZLR_mo85VGP4NE1fqsSy5dO9RmmXE2w'
]
...
!(async () => {
    ...
    for (let i = cookiesArr.length - 1; i >= 0; i--) {
        if (cookiesArr[i]) {
            ...
            for (let i = 0; i < $.helpCode.length; ++i) {
                console.log(`\n开始助力 【${$.helpCode[i]}】`)
                let res = await getInfo($.helpCode[i])
                if (res && res['data'] && res['data']['bizCode'] === 0) {
                    ...
                    if (res['data']['result']['toasts'] && res['data']['result']['toasts'][0]) {
                        console.log(`助力 【${$.helpCode[i]}】:${res.data.result.toasts[0].msg}`)
                    }
                }
                        ...
                        if (data.code === 0) {
                            if (data.data && data['data']['bizCode'] === 0) {
                                if (flag){
                                    ...
                                    $.helpCode.push(data.data.result.userActBaseInfo.inviteId);
                                }
