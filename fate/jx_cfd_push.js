/**
 * #京喜财富岛提交助力码
 * cron 0 0 * * * jx_cfd_push.js
 **/

const $ = new Env("推送京喜财富岛提交助力码");
...
function submitInviteId(userName) {
    return new Promise(resolve => {
        ...
        if ($.index == 1) {
            $.shareIdMyself.push($.info.strMyShareId);
        }
        $.post(
            {
...
function createSuperAssistUser() {
    return new Promise(resolve => {
        ...
        let sharId = $.shareIdMyself[0];
        $.log(`\n【👫🏻超级助力】超级助力码：${sharId}\n${$.showLog ? '' : ''}`);
        $.get(taskUrl('user/JoinScene', `strPgtimestamp=${$.currentToken['timestamp']}&strPhoneID=${$.currentToken['phoneid']}&strPgUUNum=${$.currentToken['farm_jstoken']}&strShareId=${escape(sharId)}&dwSceneId=${sceneId}&dwType=2&_stk=_cfd_t%2CbizCode%2CdwEnv%2CdwSceneId%2CdwType%2Cptag%2Csource%2CstrPgUUNum%2CstrPgtimestamp%2CstrPhoneID%2CstrShareId%2CstrZone`), async (err, resp, data) => {
            try {
...
//助力内部好友
function createAssistUser2() {
    return new Promise(resolve => {
        const sceneIds = Object.keys($.info.SceneList);
        const sceneId = Math.min(...sceneIds);
        let sharId = $.shareIdMyself[0];
        $.log(`\n【👬普通助力】普通助力码：${sharId}\n${$.showLog ? '' : ''}`);
        $.get(taskUrl('user/JoinScene', `strShareId=${escape(sceneId)}&dwSceneId=${sceneId}`), async (err, resp, data) => {
            try {
                const {sErrMsg, data: {rewardMoney = 0} = {}} = JSON.parse(data);
                $.log(`\n【👬普通助力】助力：${sErrMsg}\n${$.showLog ? data : ''}`);
            } catch (e) {
    ...
    return new Promise(resolve => {
        ...
        $.get({url: 'https://api.ninesix.cc/api/jx-cfd'}, async (err, resp, _data) => {
            try {
                ...
                $.get(taskUrl('user/JoinScene', `strShareId=${escape($.shareIdMyself.get(0))}&dwSceneId=${sceneId}`), async (err, resp, data) => {
                    try {
                        const {sErrMsg, data: {rewardMoney = 0} = {}} = JSON.parse(data);
                        $.log(`\n【👬普通助力】助力：${sErrMsg}\n${$.showLog ? data : ''}`);
                    } catch (e) {
...
function joinGroup() {
    return new Promise(async (resolve) => {
        let groupId = $.groupIdMyself[0];
        $.log(`\n【🏝寻宝大作战】随机助力码：${groupId}\n${$.showLog ? '' : ''}`);
        if (groupId) {
            $.get(taskUrl(`user/JoinGroup`, `strGroupId=${groupId}&dwIsNewUser=${$.info.dwIsNewUser}&pgtimestamp=${$.currentToken['timestamp']}&phoneID=${$.currentToken['phoneid']}&pgUUNum=${$.currentToken['farm_jstoken']}&_stk=_cfd_t%2CbizCode%2CdwEnv%2CdwIsNewUser%2CpgUUNum%2Cpgtimestamp%2CphoneID%2Cptag%2Csource%2CstrGroupId%2CstrZone`), (err, resp, data) => {
                try {
                    const {sErrMsg} = JSON.parse(data);
                    $.log(`\n【🏝寻宝大作战】助力：${sErrMsg}\n${$.showLog ? data : ''}`);
                } catch (e) {
...
function getTokens() {
    if ($.isNode()) {
        ...
        $.tokenArr = JSON.parse($.getdata('TokenJxnc') || '[]');
    }
    if (!$.tokenArr[0]) {
        $.msg(
            $.name,
