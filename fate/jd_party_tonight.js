...
================Loon==============
[Script]
cron "7 7 * * *" script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_bean_home.js, tag=沸腾之夜
 */
const $ = new Env('沸腾之夜');
...
function help(inviteCode) {
    return new Promise(resolve => {
        ...
        $.post(taskUrl('partyTonight_assist', body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`【助力】${$.name} API请求失败，请检查网路重试`)
                } else {
                    if (safeGet(data)) {
                        data = JSON.parse(data);
