...
[task_local]
#签到领现金
0 0-3,6 * * * https://gitee.com/lxk0301/jd_scripts/raw/master/jd_push_cash.js, tag=签到领现金, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

================Loon==============
[Script]
cron "0 0-3,6 * * *" script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_push_cash.js,tag=签到领现金

===============Surge=================
签到领现金 = type=cron,cronexp="0 0-3,6 * * *",wake-system=1,timeout=3600,script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_push_cash.js

============小火箭=========
签到领现金 = type=cron,script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_push_cash.js, cronexpr="0 0-3,6 * * *", timeout=3600, enable=true
 */
...
function cashPush(helpInfo) {
    return new Promise(resolve => {
        console.log("助力码:", helpInfo.inviteCode + "," + helpInfo.shareDate + "\n");
        $.get({
            url: "http://api.tyh52.com/act/set/jd_cash/" + helpInfo.inviteCode + "," + helpInfo.shareDate
        }, (err, resp, data) => {
            try {
                if (data) {
                    data = JSON.parse(data);
                    if (data.code == 1) {
                        console.log("提交自己的邀請碼成功");
                    } else {
                        console.log("已经提交过自己的邀请码");
                    }
                }
            } catch (e) {
                $.logErr(e, resp);
