 ...
 * 打开京喜农场，添加下面的重写，手动完成任意任务，提示获取cookie成功，然后退出跑任务脚本
  ...
  quanx:
  [task_local]
  0 8,11,17 * * * https://raw.githubusercontent.com/whyour/hundun/master/quanx/jd_jxnc.js, tag=京喜农场, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxnc.png, enabled=true
  [rewrite_local]
  ...
  cron "0 8,11,17 * * *" script-path=https://raw.githubusercontent.com/whyour/hundun/master/quanx/jd_jxnc.js, tag=京喜农场

  surge:
  [Script]
  京喜农场 = type=cron,cronexp=0 8,11,17 * * *,timeout=60,script-path=https://raw.githubusercontent.com/whyour/hundun/master/quanx/jd_jxnc.js,
...
!(async () => {
    ...
    for (let i = 0; i < $.cookieArr.length; i++) {
        ...
        if ($.currentCookie) {
            ...
            for (let extra of $.helpExtra) {
                console.log(`\n${$.UserName} 去参加团 ${extra.smp}`);
                await assistUser(extra);
                await $.wait(500);
            }
        }
    }
...
function assistUser(extra) {
    return new Promise(resolve => {
        $.get(
            taskUrl('help', `active=${extra.active}&joinnum=${extra.joinnum}&smp=${extra.smp}`),
            async (err, resp, data) => {
                try {
                    const res = data.match(/try\{whyour\(([\s\S]*)\)\;\}catch\(e\)\{\}/)[1];
