...
[task_local]
#京喜财富岛
56 59 * * * * https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jx_cfd_exchange.js, tag=京喜财富岛, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxcfd.png, enabled=true

================Loon==============
[Script]
cron "56 59 * * * *" script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jx_cfd_exchange.js,tag=京喜财富岛

===============Surge=================
京喜财富岛 = type=cron,cronexp="1 * * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jx_cfd_exchange.js

============小火箭=========
京喜财富岛 = type=cron,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jx_cfd_exchange.js, cronexpr="1 * * * *", timeout=3600, enable=true
 */
...
$.exchangeNum = process.env.cfdExchangeNum ?? 0; // 兑换现金的 0:100  1:100,1  2: 100,1,0.5
...
function exchangePrize( body = ''){
    return new Promise((resolve) => {
        $.get(taskUrl(`user/ExchangePrize`, body), (err, resp, data) => {
            try {
                if (err) {
...
// blkpool: 'jxcfd2_risk_yhq_2021',
// hongbaopool: 'jxcfd2_exchange_hb_202110',
// blkswpool: 'jxcfd2_risk_sw_2021',
// 兑换任务100
function exchangePrize100( body = ''){
    body = `dwType=3&dwLvl=3&ddwPaperMoney=100000&strPoolName=jxcfd2_exchange_hb_202111&strPgUUNum=${token['farm_jstoken']}&strPgtimestamp=${token['timestamp']}&strPhoneID=${token['phoneid']}`
    return new Promise((resolve) => {
        $.get(taskUrl2(`user/ExchangePrize`, body), (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} 兑换任务100请求失败，请检查网路重试`)
                } else {
                    data = JSON.parse(data);
                    if (data.iRet === 0 || data.sErrMsg === 'success') {
                        console.log(noteMsg);
                        let noteMsg = `\n 【京东账号${$.index}】${$.UserName} 兑换 ${data.strAwardDetail.strName} 成功。`
                        notify.sendNotify(`京喜财富岛红包兑换`, noteMsg);
                    } else {
                        console.log(`\n 兑换失败: ${data.sErrMsg}`);
                    }
                }
...
// 兑换任务111
function exchangePrize111( body = ''){
    body = `dwType=3&dwLvl=17&ddwPaperMoney=111000&strPoolName=jxcfd2_exchange_hb_202110&strPgUUNum=${token['farm_jstoken']}&strPgtimestamp=${token['timestamp']}&strPhoneID=${token['phoneid']}`
    return new Promise((resolve) => {
        $.get(taskUrl(`user/ExchangePrize`, body), (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} 兑换红包 API请求失败，请检查网路重试`)
                } else {
                    data = JSON.parse(data);
                    if (data.iRet === 0 || data.sErrMsg === 'success') {
...
// 兑换任务1
function exchangePrize1( body = ''){
    body = `dwType=3&dwLvl=16&ddwPaperMoney=1000&strPoolName=jxcfd2_exchange_hb_202110&strPgUUNum=${token['farm_jstoken']}&strPgtimestamp=${token['timestamp']}&strPhoneID=${token['phoneid']}`
    return new Promise((resolve) => {
        $.get(taskUrl(`user/ExchangePrize`, body), (err, resp, data) => {
            try {
                if (err) {
...
//自动兑换
async function exchangeState(dwType = 2){
    return new Promise((resolve) => {
        $.get(taskUrl(`user/ExchangeState`, `dwType=${dwType}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} 兑换列表 API请求失败，请检查网路重试`)
                } else {
                    data = JSON.parse(data);
                    if (data.iRet === 0 || data.sErrMsg === 'success') {
                        console.log(data);
...
function taskUrl2(function_path, body = '', dwEnv = 7) {
    let url = `${JD_API_HOST}jxbfd/${function_path}?strZone=jxbfd&bizCode=jxbfd&source=jxbfd&dwEnv=${dwEnv}&_cfd_t=${Date.now()}&ptag=138631.135.2&${body}&_stk=_cfd_t%2CbizCode%2CddwPaperMoney%2CdwEnv%2CdwLvl%2CdwType%2Cptag%2Csource%2CstrPgUUNum%2CstrPgtimestamp%2CstrPhoneID%2CstrPoolName%2CstrZone&_ste=1`;
    url += `&h5st=${decrypt(Date.now(), '', '', url)}&_=${Date.now() + 2}&sceneval=2&g_login_type=1&g_ty=ls`;
	// console.log(url);
    return {
        url,
