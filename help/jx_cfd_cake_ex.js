...
async function perl_auto() {
    try {
        await refresh_perl()
        if (!$.perl_data) {
            console.error('获取兑换列表失败！！！\n');
            return
        }
        const ddwVirHb = $.perl_data.ddwVirHb
        console.log('当前余额:',ddwVirHb/100)
        let prizes = [...$.perl_data.exchangeInfo.prizeInfo].filter(prize => {
            const strPrizeName = prize.strPrizeName;
        ...
        for (let i = 0; i < prizes.length; i++) {
            ...
            console.log(`${(new Date()).Format("hh:mm:ss | S")}兑换面额:`, prize.strPrizeName)
