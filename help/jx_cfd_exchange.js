...
[task_local]
#京喜财富岛
1 * * * * https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jx_cfd_exchange.js, tag=京喜财富岛, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxcfd.png, enabled=true

================Loon==============
[Script]
cron "1 * * * *" script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jx_cfd_exchange.js,tag=京喜财富岛
...
function exchangePrize100( body = ''){
    body = `dwType=3&dwLvl=3&ddwPaperMoney=100000&strPoolName=jxcfd2_exchange_hb_202110&strPgUUNum=${token['farm_jstoken']}&strPgtimestamp=${token['timestamp']}&strPhoneID=${token['phoneid']}`
    return new Promise((resolve) => {
        $.get(taskUrl(`user/ExchangePrize`, body), (err, resp, data) => {
            try {
                if (err) {
