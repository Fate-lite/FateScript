...
[task_local]
#jd免费水果
0 0 * * * jd_fruit_help2.js, tag=东东农场, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdnc.png, enabled=true
=========================Loon=============================
[Script]
cron "0 0 * * *" script-path=jd_fruit_help2.js,tag=东东农场

=========================Surge============================
东东农场 = type=cron,cronexp="0 0 * * *",wake-system=1,timeout=3600,script-path=jd_fruit_help2.js

=========================小火箭===========================
东东农场 = type=cron,script-path=jd_fruit_help2.js, cronexpr="0 0 * * *", timeout=3600, enable=true
...
let shareCodes = ['40cf6440db1943e2a9d1a6ae28366100@948da82d3b0b44dcbad90353cd177408@31c5eed3c8b4448ca04db3a04ffe6d21']
...
async function jdFruit() {
    ...
    try {
        ...
        if ($.farmInfo.farmUserPro) {
            ...
            if (addFlag) {
                newHelpCodes.push($.farmInfo.farmUserPro.shareCode);
            }
...
async function masterHelpShare() {
    ...
    console.log(`格式化后的助力码::${JSON.stringify(newHelpCodes)}\n`);
...
function readShareCode() {
    return new Promise(async resolve => {
        $.get({
            url: `https://cdn.nz.lu/api/farm/${randomCount}`,
            headers: {'Host': 'api.sharecode.ga'},
            timeout: 10000
        }, (err, resp, data) => {
            try {
                if (err) {
