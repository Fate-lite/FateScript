...
[task_local]
#京喜财富岛互助
1 0,5 * * * https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_cfd.js, tag=京喜财富岛互助, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxcfd.png, enabled=true

================Loon==============
[Script]
cron "1 0,5 * * *" script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_cfd.js,tag=京喜财富岛互助

===============Surge=================
京喜财富岛互助 = type=cron,cronexp="1 0,5 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_cfd.js

============小火箭=========
京喜财富岛互助 = type=cron,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_cfd.js, cronexpr="1 0,5 * * *", timeout=3600, enable=true
 */
...
!(async () => {
    ...
    for (let i = 0; i < 100; i++) {
        ...
        if ($.index > $.runHelpUser){
            console.log(`开始互助: \n`);
            for (let j = 0; j < $.selfHelpCode.length && $.canHelp; j++) {
                console.log(`账号${$.UserName} 去助力 ${$.selfHelpCode[j]}`)
                $.delcode = false
                await helpByStage($.selfHelpCode[j])
                await $.wait(1000)
                if ($.delcode) {
                    $.selfHelpCode.splice(j, 1)
                    j--
                }
            }
        }
