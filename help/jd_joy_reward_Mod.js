...
async function joyReward() {
    try {
        ...
        if (new Date().getMinutes() == 59) {
            ...
            noontime = new Date().getSeconds();
            let millTime = new Date().getMilliseconds();
            if (noontime < 59) {
                noontime = new Date().getSeconds() + 1;
                sleeptime = (starttime - noontime) * 1000 + 1000 - millTime;
                ...
                for (let item of data[giftSaleInfos]) {
                    if (item.giftType === 'jd_bean') {
                        ...
                        if (salePrice && rewardNum == 500) {
                            if (!saleInfoId)
                                continue;
                            console.log(`开始兑换${rewardNum}京豆,时间 ${(new Date()).Format("hh:mm:ss | S")}`);
                            await exchange(saleInfoId, 'pet');
                            console.log(`结束兑换API后时间 ${(new Date()).Format("hh:mm:ss | S")}`);
