...
====================================小火箭=============================
京东全民开红包 = type=cron,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_redP.js, cronexpr="1 0,2,18 * * *", timeout=3600, enable=true
 */
...
async function red() {
    ...
    if ($.h5activityIndex && $.h5activityIndex.data && $.h5activityIndex.data.biz_code === 10002) {
        //可发起拆红包活动
        await h5launch();
        if ($.redPacketIdNew != null){
            makeShareCodes($.redPacketIdNew);
        }
        ...
        if (redPacketId) {
            $.redPacketId.push(redPacketId) // data.data.result.redPacketId
