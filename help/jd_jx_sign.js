/*
京喜签到
cron 0 0 * * * jd_jx_sign.js
更新时间：2021-7-31
活动入口：京喜APP-我的-京喜签到已支持IOS双京东账号,Node.js支持N个京东账号
 */
...
!(async () => {
    ...
    for (let i = $.runHelpUser; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            cookie = cookiesArr[i];
            $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
            $.index = i + 1;
                    ...
                    switch (type) {
                        case 1:
                            if (status === 1) {
                                ...
                                if (helpNum == $.domax) {
                                    max = true;
                                    console.log(`${$.nickName || $.UserName}无需帮助了`)
                                }
                                if ($.index  <= $.runHelpUser){
                                    console.log(`【${$.nickName || $.UserName}的签到互助码】:${smp}`)
                                    $.shareCodes.push({
                                        'use': $.UserName,
                                        'smp': smp,
                                        'num': helpNum || 0,
