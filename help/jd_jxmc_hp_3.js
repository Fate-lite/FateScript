...
async function takeGetRequest(type) {
    ...
    switch (type) {
        ...
        case 'GetInviteStatus':
            url = `https://m.jingxi.com/jxmc/operservice/GetInviteStatus?channel=7&sceneid=1001&activeid=${$.activeid}&activekey=${$.activekey}&jxmc_jstoken=${token['farm_jstoken']}&timestamp=${token['timestamp']}&phoneid=${token['phoneid']}`;
            url += `&_stk=${getStk(url)}`;
            url += `&_ste=1&h5st=${decrypt(Date.now(), '', '', url)}&_=${Date.now() + 2}&sceneval=2&g_login_type=1&callback=jsonpCBK${String.fromCharCode(Math.floor(Math.random() * 26) + "A".charCodeAt(0))}&g_ty=ls`;
        ...
        case 'help':
            ...
            break;
        case 'helpHb':
            url = `https://m.jingxi.com/jxmc/operservice/InviteEnroll?sharekey=${$.codeHb}&channel=7&sceneid=1001&activeid=${$.activeid}&activekey=${$.activekey}&jxmc_jstoken=${token['farm_jstoken']}&timestamp=${token['timestamp']}&phoneid=${token['phoneid']}&_stk=channel%2Csceneid%2Csharekey&_ste=1`;
            url += `&h5st=${decrypt(Date.now(), '', '', url)}&_=${Date.now() + 2}&sceneval=2&g_login_type=1&callback=jsonpCBK${String.fromCharCode(Math.floor(Math.random() * 26) + "A".charCodeAt(0))}&g_ty=ls`;
            myRequest = getGetRequest(`help`, url);
            break;
    ...
    switch (type) {
        case 'GetHomePageInfo':
            ...
            if (data.ret === 0) {
                ...
                console.log(`获取活动信息异常：${data.message}\n\n`);
            }
            break;
        case 'GetInviteStatus':
            data = JSON.parse(data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1]);
        ...
        case 'helpHb':
            ...
            if (data.ret === 0) {
                if (data.data.result === 0) {
                    console.log(`助力成功`);
                } else if (data.data.result === 2707) {
                    console.log(`不能助力自己`);
                } else if (data.data.result === 2708) {
                    console.log(`该好友助力已满`);
                    $.delcode = true;
                } else if (data.data.result === 2709) {
                    console.log(`助力次数已用完`);
                    $.canHelp = false;
                } else if (data.data.result === 2710) {
                    console.log(`已经助力过此好友`);
                } else {
                    console.log(JSON.stringify(data))
                }
