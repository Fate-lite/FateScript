...
let inviteCodes = []
$.shareCodesArr = ['ZXASTT020vPl6RxgY9lPKIxrynfcPFjRWn6u7zB55awQ'];
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            for (let i = 0; i < inviteIdSelf.length && true; ++i) {
                console.log(`\n开始助力 【${inviteIdSelf[i]}】`)
                let res = await travel_collectScore_h(inviteIdSelf[i])
                if (res.data.bizCode == -1002) {
                    console.log("\n黑号了哦\n");
                    canHelp = false;
                    break;
                }
...
function travel_collectScore_h(inviteId) {
    let body = {
        "inviteId": inviteId,
        "ss": {"extraData": {"log": "", "sceneid": "HYGJZYh5"}, "secretp": secretp, "random": randomString(6)}
    }
    return new Promise((resolve) => {
        $.post(taskPostUrl("travel_collectScore", body), async (err, resp, data) => {
            try {
